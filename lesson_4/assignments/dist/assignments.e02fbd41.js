...
var _favoritesJs = require("./favorites.js");
async function init() {
    try {
        _uiJs.showLoading();
        const breeds = await _apiJs.fetchBreeds();
        _uiJs.populateBreeds(breeds);
        await _favoritesJs.loadFavorites();
        if (breeds && breeds.length > 0) {
            _uiJs.breedSelect.value = breeds[0].id;
            await loadBreedCats();
        } else _uiJs.infoDump.innerHTML = "<p>No breeds available.</p>";
    } catch (error) {
        _uiJs.showError("Failed to load the app!");
        console.error(error);
    }
}
async function loadBreedCats() {
    const breedId = _uiJs.breedSelect.value;
    if (!breedId) {
        _uiJs.infoDump.innerHTML = "<p>Please select a breed to see cats!</p>";
        return;
    }
    // START LOADING
    _uiJs.showLoading();
    _uiJs.updateProgress("20%");
    const cats = await _apiJs.fetchCatsByBreed(breedId);
    // DATA ARRIVED
    _uiJs.updateProgress("80%");
    _uiJs.displayCats(cats);
    // FINISHED
    _uiJs.updateProgress("100%");
    // initialize carousel if bootstrap present
    const carouselEl = document.getElementById("carouselExampleControls");
    try {
        if (window.bootstrap && carouselEl) window.bootstrap.Carousel.getOrCreateInstance(carouselEl, {
            interval: false
        });
    } catch (e) {}
...
async function toggleFavoriteById(imageId, btnElement) {
    if (!imageId) return;
    const isFav = _favoritesJs.isFavorite(imageId);
    if (btnElement) btnElement.disabled = true;
    if (isFav) {
        const ok = await _apiJs.removeFavoriteByImageId(imageId);
        if (ok) {
            _favoritesJs.removeFromFavorites(imageId);
            btnElement.textContent = "\uD83E\uDD0D Add to Favorites";
        }
    } else {
        const added = await _apiJs.addFavorite(imageId);
        if (added) _favoritesJs.addToFavorites(imageId);
    }
...
async function showFavorites() {
    _uiJs.showLoading();
    _uiJs.updateProgress("20%");
    const favList = await _apiJs.fetchFavorites();
    _uiJs.updateProgress("100%");
    if (!favList.length) {
        _uiJs.infoDump.innerHTML = "<p>No favorites yet \u2764\uFE0F</p>";
        return;
    }
...
// safe event hookup
if (_uiJs.breedSelect) _uiJs.breedSelect.addEventListener("change", loadBreedCats);
else console.error("breedSelect element not found");
if (_uiJs.getFavoritesBtn) _uiJs.getFavoritesBtn.addEventListener("click", showFavorites);
else console.error("getFavouritesBtn element not found");
// Start app
init();

},{"./api.js":"lJgrb","./ui.js":"eQTlu","./favorites.js":"1wV1J"}],"lJgrb":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "fetchBreeds", ()=>fetchBreeds);
parcelHelpers.export(exports, "fetchCatsByBreed", ()=>fetchCatsByBreed);
parcelHelpers.export(exports, "fetchFavorites", ()=>fetchFavorites);
...
parcelHelpers.export(exports, "removeFavoriteByImageId", ()=>removeFavoriteByImageId);
...
async function fetchCatsByBreed(breedId, limit = 10) {
    try {
...
async function removeFavorite(favouriteId) {
    try {
...
async function removeFavoriteByImageId(imageId) {
    try {
        const res = await axios.get("/favourites", {
            params: {
                sub_id: SUB_ID
...
parcelHelpers.export(exports, "infoDump", ()=>infoDump);
parcelHelpers.export(exports, "progressBar", ()=>progressBar);
parcelHelpers.export(exports, "getFavoritesBtn", ()=>getFavoritesBtn);
//Fill the breed dropdown with options
parcelHelpers.export(exports, "populateBreeds", ()=>populateBreeds);
//show cat pictures in a carousel
parcelHelpers.export(exports, "displayCats", ()=>displayCats);
...
function populateBreeds(breeds) {
    ...
    if (breeds.length > 0) breedSelect.value = breeds[0].id;
}
function displayCats(images) {
    ...
    images.forEach((image)=>{
      ...
      <button class="favorite-btn" data-img-id="${image.id}">\u{2764}\u{FE0F} Favorite</button>
    `;
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"1wV1J":[function(require,module,exports,__globalThis) {
...
//Get all favorites
parcelHelpers.export(exports, "getFavoritesList", ()=>getFavoritesList);
var _apiJs = require("./api.js");
//Keep track of favorites in memory
let favorites = new Set();
async function loadFavorites() {
    const data = await (0, _apiJs.fetchFavorites)();
    data.forEach((fav)=>{
        favorites.add(fav.image_id);
    });
    return data;
...
},{"./api.js":"lJgrb","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["kxwl6","jOXmm"], "jOXmm", "parcelRequire6d5d", {})
